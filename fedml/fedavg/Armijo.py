...
# def compute_grad_norm(grad_list):
#     grad_norm = torch.tensor(0.0, device=grad_list[0].device if grad_list[0] is not None else 'cpu')
#     for g in grad_list:
#         if g is None:
#             continue
#         grad_norm += torch.sum(torch.mul(g, g))
...
def check_armijo_conditions(step_size, step_size_old, loss, grad_norm, loss_next, c, beta_b):
    found = 0
...
class Sls(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        line_search_fn (float, optional): the condition used by the line-search to find the 
                    step-size (default: Armijo)
    """
    def __init__(self, params, n_batches_per_epoch=500, init_step_size=1, c=0.1, beta_b=0.9, gamma=2.0, beta_f=2.0,
                 reset_option=1, eta_max=10, bound_step_size=True, line_search_fn="armijo"):
        defaults = dict(n_batches_per_epoch=n_batches_per_epoch, init_step_size=init_step_size, c=c, beta_b=beta_b,
                        gamma=gamma, beta_f=beta_f, reset_option=reset_option, eta_max=eta_max, bound_step_size=bound_step_size,
                        line_search_fn=line_search_fn)
        super().__init__(params, defaults)
        self.state['step'] = 0
        self.state['step_size'] = init_step_size
    ...
    def step(self, loss):
        ...
        for group in self.param_groups:
            ...
            grad_current = get_grad_list(params)
            grad_norm = compute_grad_norm(grad_current)
            step_size = reset_step(step_size=batch_step_size, n_batches_per_epoch=group['n_batches_per_epoch'],
                                   gamma=group['gamma'], reset_option=group['reset_option'], init_step_size=group['init_step_size'])
            with torch.no_grad():
                if grad_norm >= 1e-8:
                    found = 0
                    step_size_old = step_size
                    for e in range(100):
                        ...
                        self.state['n_forwards'] += 1
                        if group['line_search_fn'] == "armijo":
                            armijo_results = check_armijo_conditions(step_size=step_size, step_size_old=step_size_old,
                                                                     loss=loss.item(), grad_norm=grad_norm, loss_next=loss_next,
                                                                     c=group['c'], beta_b=group['beta_b'])
                            found, step_size, step_size_old = armijo_results
                            if found == 1:
                                break
                        elif group['line_search_fn'] == "goldstein":
                            goldstein_results = check_goldstein_conditions(step_size=step_size, loss=loss.item(),
                                                                           grad_norm=grad_norm, loss_next=loss_next, c=group['c'],
                                                                           beta_b=group['beta_b'], beta_f=group['beta_f'],
                                                                           bound_step_size=group['bound_step_size'], eta_max=group['eta_max'])
                            found = goldstein_results["found"]
                            step_size = goldstein_results["step_size"]
                            if found == 3:
                                break
